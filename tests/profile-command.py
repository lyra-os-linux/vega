...
with tempfile.TemporaryDirectory(prefix='sheliak-pins-profile-contract-' if args.suite_helper else 'vega-profile-contract-') as tmp:
    ...
    check('Ubuntu workspace button visible', not read('hide-workspace-button'))
    check('Ubuntu menus and search hidden', all(not read(key) for key in
        ['show-applications-menu', 'show-places-menu', 'show-system-menu', 'show-search-menu']))
    settings('set', 'icon-size', '44')
    choose('windows10')
    check('Windows 10 keeps previous workspace preference', read('hide-workspace-button'))
    check('Windows 10 bottom/start', read('position') == 'bottom' and read('extended-content-alignment') == 'start')
    settings('set', 'icon-size', '36')
    choose('windows11')
    check('Windows 11 keeps previous workspace preference', read('hide-workspace-button'))
    check('Windows 11 bottom/center', read('position') == 'bottom' and read('extended-content-alignment') == 'center')
    settings('set', 'icon-size', '40')
    choose('macos')
    check('Lyra Floating keeps previous workspace preference', read('hide-workspace-button'))
    ...
    check('Lyra workspace preference restored', read('hide-workspace-button'))
    choose('windows10')
    check('Windows 10 customized size restored', read('icon-size') == 36)
    choose('ubuntu')
    check('Ubuntu customized size restored', read('icon-size') == 44)
    check('Ubuntu workspace button restored', not read('hide-workspace-button'))
